...
class Score:
    ...
    def _parse_from_bsoup(cls, soup, ptype: str = "Recent Plays") -> "List[Score]":
        ...
        top_div = next(
            (
                div
                for div in divs
                if div.find("b", style="color: #EB2F96;")
                and div.find("b").text == ptype
            ),
            None,
        )
        ...
        for li in top_div.find_all(
            ...
            if div:
                ...
                score.pp = (
                    ...
                    if details_lines[1].replace("pp: ", "") != "None"
                    else 0.0
                )
                score.score = int(
                ...
                hash_content = (
                    inner_div.find("span", style="display:none;")
                    .contents[0]
                    .split(":")[1]
                    .replace("}", "")
                )
                score.md5 = hash_content
            scores.append(score)
        return scores
    ...
    def _from_api_response(cls, data) -> "Score":
        ...
        score.mods = ModList.from_dict(data["Mods"])
